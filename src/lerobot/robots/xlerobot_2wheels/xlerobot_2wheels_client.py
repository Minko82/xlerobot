...
class XLerobot2WheelsClient(Robot):
    config_class = XLerobot2WheelsClientConfig
    name = "xlerobot_2wheels_client"

    def __init__(self, config: XLerobot2WheelsClientConfig):
    ...
    def connect(self) -> None:
        ...
        if self._is_connected:
            raise DeviceAlreadyConnectedError(
                "XLerobot2Wheels Daemon is already connected. Do not run `robot.connect()` twice."
            )
        ...
        if self.zmq_observation_socket not in socks or socks[self.zmq_observation_socket] != zmq.POLLIN:
            raise DeviceNotConnectedError("Timeout waiting for XLerobot2Wheels Host to connect expired.")
    ...
    def get_observation(self) -> dict[str, Any]:
        ...
        present wheel speeds (converted to body-frame velocities: x, theta),
    ...
    def send_action(self, action: dict[str, Any]) -> dict[str, Any]:
        """Command xlerobot_2wheels to move to a target joint configuration. Translates to motor space + sends over ZMQ
        ...
        if not self._is_connected:
            raise DeviceNotConnectedError(
                "XLerobot2Wheels is not connected. You need to run `robot.connect()`."
            )
    ...
    def disconnect(self):
        ...
        if not self._is_connected:
            raise DeviceNotConnectedError(
                "XLerobot2Wheels is not connected. You need to run `robot.connect()` before disconnecting."
            )
