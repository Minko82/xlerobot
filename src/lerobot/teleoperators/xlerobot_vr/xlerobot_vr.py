...
class SimpleTeleopArm:
    ...
    def handle_vr_input(self, vr_goal, gripper_state):
        ...
        try:
            ...
            alpha = 0.1
            self.target_positions["shoulder_lift"] = (1 - alpha) * self.target_positions.get(
                "shoulder_lift", 0.0
            ) + alpha * joint2_target
            self.target_positions["elbow_flex"] = (1 - alpha) * self.target_positions.get(
                "elbow_flex", 0.0
            ) + alpha * joint3_target
        except Exception as e:
...
class XLerobotVRTeleop(Teleoperator):
    ...
    def is_connected(self) -> bool:
        ...
        return (
            ...
            and (self.vr_thread is not None and self.vr_thread.is_alive())
        )
    ...
    def connect(self, calibrate: bool = True, robot=None) -> None:
        ...
        try:
            ...
            self.vr_thread = threading.Thread(
                target=lambda: asyncio.run(self.vr_monitor.start_monitoring()), daemon=True
            )
            ...
            self._connected = False
            raise RuntimeError(f"Failed to connect to VR: {e}") from e

    def calibrate(self, robot_obs: dict | None = None) -> None:
        ...
        try:
            ...
            self.left_arm = SimpleTeleopArm(
                LEFT_JOINT_MAP, robot_obs, self.kin_left, prefix="left", kp=self.config.kp
            )
            self.right_arm = SimpleTeleopArm(
                RIGHT_JOINT_MAP, robot_obs, self.kin_right, prefix="right", kp=self.config.kp
            )
    ...
    def get_action(self, robot_obs: dict | None = None, robot=None) -> dict[str, Any]:
    ...
    def _update_events_inline(self, left_goal):
        ...
        try:
            ...
            logger.debug(
                f"Low frequency event update failed: {e}"
            )  # Downgrade to debug to avoid disrupting main flow
    ...
    def disconnect(self) -> None:
        ...
        if not self.is_connected:
            raise RuntimeError("XLerobot VR is not connected.")
...
class VREventHandler:
    ...
    def _process_left_controller(self, metadata):
        ...
        elif thumb_x < -self.threshold or self.events["rerecord_episode"]:
