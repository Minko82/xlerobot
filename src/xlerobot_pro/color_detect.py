...
def detect_color(bgr: np.ndarray, color: str, min_area: int = 100, blur_ksize: int = 5) -> list[Detection]:
    ...
    for cnt in contours:
        ...
        cx = int(M["m10"] / M["m00"])
        cy = int(M["m01"] / M["m00"])
        detections.append(Detection(centroid_px=(cx, cy), area=area, bbox=(x, y, w, h), contour=cnt))

    detections.sort(key=lambda d: d.area, reverse=True)
    return detections


def detection_to_xyz(det: Detection, depth_m: np.ndarray, intrinsics: dict, patch: int = 5) -> np.ndarray:
...
def detect_object(color: str = "red", captures_dir: str | Path | None = None) -> np.ndarray:
