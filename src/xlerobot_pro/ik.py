...
class IK_SO101:
    def __init__(self) -> None:
        ...
        lock_ids = [i for i in range(1, full_model.njoints) if full_model.names[i] not in _ARM_JOINTS]
    ...
    def _run_ik_from_seed(
        ...
        pin.forwardKinematics(self.model, self.data, self.configuration.q)
        pin.updateFramePlacements(self.model, self.data)
        final_error = np.linalg.norm(target_transform.translation - self.data.oMf[ee_frame_id].translation)
    ...
    def generate_ik(
        ...
        if len(best_traj) >= max_timesteps:
            print(
                f"IK did not converge: error={best_error * 1000:.1f}mm, steps={len(best_traj)}/{max_timesteps}"
            )
            return []
...
if __name__ == "__main__":
    ...
    print("Generating IK trajectory...")
