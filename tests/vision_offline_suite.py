...
Also run automatically by pytest via tests/test_vision.py when pinocchio
and pink are installed.
"""
...
def test_mjcf_loading():
    ...
    check("Model loads successfully", model.njoints > 1, f"{model.njoints} joints (including universe)")
    ...
    for fname in expected_frames:
        ...
        check(f"Frame '{fname}' in model", fid < model.nframes, f"id={fid}")
...
def test_fk_sanity(model: pin.Model):
    ...
    check(
        "Base not at origin (is mounted)",
        np.linalg.norm(base_pos) > 0.01,
    ...
    check("EE within reasonable arm reach", 0.05 < dist < 0.5, f"dist={dist:.4f}")
    ...
    check(
        "Camera above Base (Z)", cam_pos[2] > base_pos[2], f"cam_z={cam_pos[2]:.4f}, base_z={base_pos[2]:.4f}"
    )
...
def test_frame_transform():
    ...
    check(
        "Horizontal distance > 0", np.sqrt(bx**2 + by**2) > 0.01, f"horiz_dist={np.sqrt(bx**2 + by**2):.4f}"
    )
    ...
    check(
        "Shifting camera X changes Base position",
        np.linalg.norm(shift) > 0.01,
    ...
    z_shift = bz3 - bz
    check("Camera +Y (down) lowers Base Z", z_shift < -0.01, f"z_shift={z_shift:.4f}")
    ...
    check("Head pan changes horizontal position", pan_shift > 0.01, f"|xy_shift|={pan_shift:.4f}")
...
def test_ik_convergence():
    ...
    for name, target in targets:
        ...
        check(f"IK '{name}'", converged, f"error={error * 1000:.1f}mm, steps={len(traj)}")
    ...
    if len(traj_far) > 0:
        ...
        error = np.linalg.norm(ee_world - target_world)
        check(
            "IK unreachable target doesn't claim success",
            error > 0.1,
...
def test_ik_determinism():
    ...
    if len(traj1) > 0 and len(traj2) > 0:
        ...
        check("Same target → same final config", diff < 0.01, f"|q1-q2|={diff:.6f} rad")

        len_ratio = len(traj2) / len(traj1) if len(traj1) > 0 else float("inf")
        check("Trajectory lengths similar", 0.5 < len_ratio < 2.0, f"len1={len(traj1)}, len2={len(traj2)}")
    else:
        check(
    ...
    if len(traj_a) > 0 and len(traj_b) > 0:
        ...
        check("Fresh instances → identical results", diff_ab < 1e-10, f"|qa-qb|={diff_ab:.2e}")
...
def test_motor_mapping():
    ...
    check(
        "MJCF all-zero: motor shoulder_lift=90°",
        abs(zero_motor[1] - 90.0) < 0.01,
        f"got {zero_motor[1]:.2f}°",
    )
    check(
        "MJCF all-zero: motor elbow_flex=-90°",
        abs(zero_motor[2] - (-90.0)) < 0.01,
        f"got {zero_motor[2]:.2f}°",
    )
...
def test_full_pipeline_roundtrip():
    ...
    if len(traj) > 0:
        ...
        check("Full pipeline IK error < 5mm", ik_error < 0.005, f"{ik_error * 1000:.1f}mm")
        ...
        check("Motor mapping round-trip lossless", mapping_error < 1e-10, f"error={mapping_error:.2e}")
